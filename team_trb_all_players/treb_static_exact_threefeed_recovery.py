...
def controls(cur):
 ...
 pm={(gid(r.game_id),int(r.team_id),sid(r.player_id)):r for r in p.itertuples(index=False)}
 tm={(gid(r.game_id),int(r.team_id)):r for r in t.itertuples(index=False)}
 return pm,tm
def eq(old,new,fs):
...
def main():
 ...
 reg=pd.read_csv(pick(cur,'NEXT_RESIDUAL_SHARED_GAME_REGISTRY.csv'),low_memory=False);games,np,nt=targets(reg,season)
 qa={'status':'PASS','season':season,'upstream_commit':UPSTREAM_COMMIT,'target_games':len(games),'required_player_primitives':sum(map(len,np.values())),'required_team_primitives':sum(map(len,nt.values())),'controls_checked':0,'control_mismatches':0,'recovered_player_primitives':0,'recovered_team_primitives':0,'game_failures':[],'sources':{},'integrity':{'exact_existing_build_game_engine':True,'pinned_static_nba_v3_pbp_only':True,'empirical_model_used':False,'rounded_percentage_backsolve_used':False,'opponent_rebound_inference_used':False,'partial_tenure_team_subtraction_used':False,'promotion_performed':False}}
 if not games:
  qa['status']='NO_TARGETS';(out/'QA.json').write_text(json.dumps(qa,indent=2)+'\n');return 0
 pm,tm=controls(cur);cp=[];ct=[]
 with tempfile.TemporaryDirectory(prefix='treb_up_') as gd,tempfile.TemporaryDirectory(prefix='treb_arc_') as td:
  try:
   ...
   nr,qa['sources']['nbastats']=archive_df(repo,tmp,'nbastats',year,games)
   vr,qa['sources']['nbastatsv3']=archive_df(repo,tmp,'nbastatsv3',year,games)
   pr,qa['sources']['pbpstats']=archive_df(repo,tmp,'pbpstats',year,games)
  ...
  for g in sorted(games):
   if g not in ng or g not in vg or g not in pg:
    qa['game_failures'].append({'game_id':g,'status':'SOURCE_SET_GAP','nba':g in ng,'v3':g in vg,'pbp':g in pg});continue
   try: tr,pl,audit=exact.build_game(g,ng[g],vg[g],pg[g])
   except Exception as e:
    qa['game_failures'].append({'game_id':g,'status':'RECONSTRUCTION_FAIL','error':f'{type(e).__name__}: {e}'});continue
   mm=[]
   for z in tr:
    k=(g,int(z['team_id']));old=tm.get(k)
    if old is not None:
     qa['controls_checked']+=1
     if not eq(old,z,TV):mm.append({'kind':'team','key':list(k)})
   for z in pl:
    k=(g,int(z['team_id']),sid(z['player_id']));old=pm.get(k)
    if old is not None:
     qa['controls_checked']+=1
     if not eq(old,z,PV):mm.append({'kind':'player','key':list(k)})
   if mm:
    qa['control_mismatches']+=len(mm);qa['game_failures'].append({'game_id':g,'status':'CONTROL_MISMATCH','examples':mm[:10]});continue
   ...
   if miss:
    qa['game_failures'].append({'game_id':g,'status':'TARGET_NOT_RECONSTRUCTED','missing':miss});continue
   prov=f'exact pinned static nba+v3+pbp production reconstruction @ {UPSTREAM_COMMIT}'
   for k in sorted(np.get(g,set())):

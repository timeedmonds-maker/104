...
# Current 40-row non-primitive tail, verified from authoritative reclosure artifact 9275527991.
# This script is diagnostic only. Any later repair/promotion must revalidate against the newest
# authoritative residual state before mutation.
VALIDATION = [
...
SKIP={'.git','node_modules','.venv','venv','__pycache__','treb_recovery_status'}
MAX_BYTES=80*1024*1024

def main():
    root=Path(sys.argv[1] if len(sys.argv)>1 else '.')
    out=Path(sys.argv[2] if len(sys.argv)>2 else 'tail_evidence'); out.mkdir(parents=True,exist_ok=True)
    targets=[]
    for s,t,p in VALIDATION: targets.append({'lane':'VALIDATION_MINUTES','season':s,'team':t,'player_id':p,'pro_id':''})
    for s,t,p,pro in TENURE: targets.append({'lane':'TENURE_IDENTITY','season':s,'team':t,'player_id':p,'pro_id':pro})
    token_to_targets={}
    for x in targets:
        for tok in (x['player_id'],x['pro_id']):
            if tok: token_to_targets.setdefault(tok,[]).append(x)
    inventory=[]; matches=[]
    for base,dirs,files in os.walk(root):
        dirs[:]=[d for d in dirs if d not in SKIP]
        for fn in files:
            p=Path(base)/fn; rel=str(p.relative_to(root)); low=rel.lower()
            if p.suffix.lower() not in EXTS or not any(k in low for k in KEYWORDS): continue
                ...
                with p.open('r',errors='ignore') as fh:
                    for lineno,line in enumerate(fh,1):
                        for tok,tgs in token_to_targets.items():
                            if tok in line:
                                for tg in tgs: matches.append({**tg,'path':rel,'line':lineno,'token':tok,'evidence':line.strip()[:1500]})
            except Exception as e: inventory[-1]['read_error']=repr(e)
    with (out/'TREB_TAIL_SOURCE_INVENTORY.csv').open('w',newline='') as f:
        fields=sorted({k for r in inventory for k in r} or {'path','bytes'}); w=csv.DictWriter(f,fieldnames=fields); w.writeheader(); w.writerows(inventory)
    ...
    qa={'baseline_authoritative_artifact_id':9275527991,'baseline_partition':{'validation':19,'tenure':21},'validation_targets':len(VALIDATION),'tenure_targets':len(TENURE),'inventory_files':len(inventory),'matches':len(matches),'targets_with_evidence':sum(c['evidence_matches']>0 for c in coverage),'promotion_allowed':False,'authoritative_drift_gate':'REQUIRED_AT_ANY_REPAIR_OR_PROMOTION','purpose':'exact-source triage only; no values promoted'}

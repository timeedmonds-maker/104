...
def legacy_labels(nba: pd.DataFrame, pbp: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for game_id, pg in pbp.groupby("GAMEID", sort=False):
        ...
        if ng.empty:
            continue
        # We need only the normalized/elapsed event stream for rebound joining;
        # no lineup inference is involved in this bridge label step.
        ...
        if len(labeled):
            rows.append(labeled[["NBA_EVENTNUM","IS_REAL_REBOUND","IS_OREB","DESCRIPTION","NBA_PLAYER1_ID","PERIOD","STARTTIME","ENDTIME"]].assign(GAMEID=int(game_id)))
        audits.append(audit)
    out=pd.concat(rows,ignore_index=True) if rows else pd.DataFrame()
    summary={k:int(sum(a.get(k,0) for a in audits)) for k in ["rebound_bearing_rows","matched_rebound_bearing_rows","unmatched_rebound_bearing_rows","ambiguous_matches","manual_join_repairs"]}
    return out,summary
...
def modern_candidate_rule(game: pd.DataFrame) -> pd.Series:
    ...
    for idx in game.index[team]:
        ...
        # Find nearest prior non-replay action and also actions at same clock.
        prior=None
        j=pos-1
        while j>=0:
            cand=indices[j]
        ...
        # Turnover/violation placeholders at the rebound clock are dead.
        ...
        # A miss before the end of a free-throw trip is not a live rebound.
        ...
        # Horn/end-of-period team rebounds are dead when no live action follows
        # in the period.  Permit small sub-second feed clock differences.
...
def main() -> int:
    ...
    # A legacy event can theoretically be represented by duplicate PBP rows;
    # collapse only when labels agree, otherwise fail.
    ...
    for gid,idx in v3.groupby("gameId",sort=False).groups.items():
    ...
    def recs(df,n=120):
        ...
        return df[cols].head(n).where(pd.notna(df[cols].head(n)),None).to_dict("records")

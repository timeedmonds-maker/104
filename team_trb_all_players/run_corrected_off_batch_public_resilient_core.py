...
def public_resilient_request_json(url: str, params: dict[str, str], attempts: int = 4) -> tuple[dict[str, Any], dict[str, Any]]:
    v4.bump("request_calls")
    path = v4.net_cache_path(url, params)
    cached = v4.read_net_cache(path)
    if cached is not None:
        v4.bump("cache_hits")
        return cached, {"ok": True, "attempt": 0, "status_code": 200, "cache_hit": True, "network_cache": str(path), "errors": [], "public_resilient": True}
    ...
    with lock:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                return payload, {"ok": True, "attempt": attempt, "status_code": status, "status_history": statuses, "elapsed_seconds": round(time.monotonic() - started, 3), "cache_hit": False, "network_cache": str(path), "errors": errors, "public_resilient": True, "recovered_after_retry": attempt > 1}
        ...
        return {}, {"ok": False, "attempt": len(statuses), "status_code": statuses[-1] if statuses else None, "status_history": statuses, "cache_hit": False, "network_cache": str(path), "errors": errors, "public_resilient": True}
...
def self_test() -> None:
    ...
    print("run_corrected_off_batch_public_resilient_core self-test PASSED")

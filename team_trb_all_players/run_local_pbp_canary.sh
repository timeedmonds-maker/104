...
commit_result () {
  git add "$RESULT" "$STATUS" 2>/dev/null || true
...
# Safety: this branch inherits the best durable 10,112 Stage2 checkpoint, but the
# canary itself writes only its own result/status files. Corrected-OFF caches are read-only.
python - <<'PY'
import json
from pathlib import Path
...
set +e
python "$BASE/local_pbp_treb_canary.py"
RC=$?
set -e
...
if [[ -s "$RESULT" ]]; then
  if (( RC == 0 )); then
    ...
    write_status "failed" "Local bulk-PBP canary completed but did not reproduce every selected window; result committed for diagnosis; no production data changed"
  fi
  commit_result
else
  write_status "error" "Canary exited before producing a result file; no production data changed"
  git add "$STATUS" && git commit -m "Record local bulk-PBP canary error [skip ci]" || true

...
def main():
 ...
 facts=collections.defaultdict(dict); fsrc=collections.defaultdict(dict); team=collections.defaultdict(dict); tsrc=collections.defaultdict(dict)
 pconf=set(); tconf=set(); shadowed=[]
 def putp(k,z,v,src,authoritative=False):
  v=float(v); q=(k,z)
  if q in pconf and not authoritative:return
  old=facts[k].get(z); os=fsrc[k].get(z)
  if old is None or authoritative:
   if old is not None and abs(old-v)>1e-9: shadowed.append({'kind':'player','key':str(k),'field':z,'authoritative':v,'stale':old,'stale_source':os})
   facts[k][z]=v; fsrc[k][z]='current_shared' if authoritative else src; pconf.discard(q); return
  if abs(old-v)<=1e-9:return
  if os=='current_shared':
   shadowed.append({'kind':'player','key':str(k),'field':z,'authoritative':old,'stale':v,'stale_source':src}); return
  facts[k].pop(z,None); fsrc[k].pop(z,None); pconf.add(q)
 def putt(k,z,v,src,authoritative=False):
  v=float(v); q=(k,z)
  if q in tconf and not authoritative:return
  old=team[k].get(z); os=tsrc[k].get(z)
  if old is None or authoritative:
   if old is not None and abs(old-v)>1e-9: shadowed.append({'kind':'team','key':str(k),'field':z,'authoritative':v,'stale':old,'stale_source':os})
   team[k][z]=v; tsrc[k][z]='current_shared' if authoritative else src; tconf.discard(q); return
  if abs(old-v)<=1e-9:return
  if os=='current_shared':
   shadowed.append({'kind':'team','key':str(k),'field':z,'authoritative':old,'stale':v,'stale_source':src}); return
  team[k].pop(z,None); tsrc[k].pop(z,None); tconf.add(q)
 # Controlling checkpoint is authoritative on every overlap.
 ...
 # Historical exact layers may fill gaps; disagreements between non-authoritative layers are quarantined field-by-field.
 ...
 while changed and rounds<12:
  ...
  for gt in sorted(set(k[:2] for k in needp)|needt):
   ...
   if gt in needt and not all(z in team.get(gt,{}) for z in REQT):
    ...
    if ok:
     ...
     if all(abs(x/5-round(x/5))<=1e-9 for x in nums):
      ...
      for z in REQT: putt(gt,z,v[z],'five_player_conservation')
      if all(z in team.get(gt,{}) and abs(team[gt][z]-v[z])<=1e-9 for z in REQT): rt[gt]=v; changed=True
   if all(z in team.get(gt,{}) for z in REQT):
    for k in sorted(x for x in needp if x[:2]==gt and fullp(x) is None):
     ...
     if valid:
      ...
      if all(z in facts.get(k,{}) and abs(facts[k][z]-v[z])<=1e-9 for z in REQP): rp[k]=v; changed=True
 for k,v in rp.items():
  gt=k[:2]
  for pz,tz in (('team_oreb_on','team_oreb'),('team_dreb_on','team_dreb'),('opponent_oreb_on','opponent_oreb'),('opponent_dreb_on','opponent_dreb')):
 ...
 with open(out/'COMPLEMENT_PLAYER.csv','w',newline='') as f:
 ...
 with open(out/'COMPLEMENT_TEAM.csv','w',newline='') as f:
 ...
 qa={'status':'PASS','rounds':rounds,'target_player_gaps':len(needp),'target_team_gaps':len(needt),'recovered_exact_player_primitives':len(rp),'recovered_exact_team_facts':len(rt),'remaining_player_gaps':sum(fullp(k) is None for k in needp),'remaining_team_gaps':sum(not all(z in team.get(k,{}) for z in REQT) for k in needt),'shadowed_current_overlaps':len(shadowed),'quarantined_historical_player_fields':len(pconf),'quarantined_historical_team_fields':len(tconf),'integrity':{'controlling_checkpoint_priority':True,'non_authoritative_disagreements_quarantined':True,'five_player_conservation_only':True,'exact_roster_ledger_seconds':True,'model_used':False,'rounded_backsolve_used':False,'opponent_inference_used':False}}

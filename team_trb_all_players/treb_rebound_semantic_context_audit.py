#!/usr/bin/env python3
"""Diagnostic-only neighbouring-event enrichment for already-proven TREB control mismatches.

This lane consumes the persisted 23 exact-control mismatches from the successful semantic audit,
re-fetches only their static NBA/PBP/V3 games, and records neighbouring NBA events. It performs no
promotion and does not need the authoritative checkpoint artifact. Any semantic rule derived from
this output must still pass a fresh broad zero-mismatch exact-control gate before promotion.
"""
...
def _ctx(r,prefix):
    if r is None:
        return {f'{prefix}_eventnum':0,f'{prefix}_elapsed':-1,f'{prefix}_description':'',f'{prefix}_player1_id':0,f'{prefix}_player2_id':0,f'{prefix}_player3_id':0}
    return {f'{prefix}_eventnum':_safe_int(r.get('EVENTNUM')),f'{prefix}_elapsed':_safe_int(r.get('ELAPSED')),f'{prefix}_description':str(r.get('DESCRIPTION_NORM','')),f'{prefix}_player1_id':_safe_int(r.get('PLAYER1_ID')),f'{prefix}_player2_id':_safe_int(r.get('PLAYER2_ID')),f'{prefix}_player3_id':_safe_int(r.get('PLAYER3_ID'))}
...
def main():
    ...
    qa['event_rows']=len(events)
    mm.to_csv(out/'MISMATCHES.csv.gz',index=False,compression='gzip')

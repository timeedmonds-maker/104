...
d.update({
  ...
  'runner': 'v11 fast repair from durable v10 inputs',
})
...
PY
  ...
  if ! git diff --cached --quiet; then
    git commit -m "Stage 1 fast checkpoint: ${phase} [skip ci]"
    git push origin "HEAD:$BRANCH"
  fi
}

echo "============================================================"
echo "TREB V11 FAST STAGE-1 REPAIR"
echo "Reuses durable historical archive + schedules; no archive refetch."
echo "============================================================"

python "$BASE/repair_remaining_overlap_sources_v11.py" --self-test
python - <<'PY'
import sys
...
python "$BASE/repair_remaining_overlap_sources_v11.py"
checkpoint "v11_source_repairs"

# Rebuild only tenure chronology from the durable normalized transaction stream.
set +e
...
with gzip.open(p,'rt',encoding='utf-8') as f:
    for line in f:
        ...
        rows.append(r)
if bad: raise SystemExit(f'V11 chronology left {len(bad)} invalid intervals')
with gzip.open(p,'wt',encoding='utf-8') as f:
    for r in rows: f.write(json.dumps(r,ensure_ascii=False)+'\n')
print('V11 RAW CHRONOLOGY BUILT', len(rows), 'windows; stale flags cleared=', stale, 'builder_rc=', sys.argv[1])
PY
checkpoint "v11_tenure_chronology"

# Schedules are already durable and validated; only recompute effective windows.
...
python "$BASE/diagnose_remaining_overlap_event_chains_v11.py"
checkpoint "v11_overlap_diagnostic"
...
print('V11 STRICT OVERLAPS=', d.get('strict_cross_team_overlap_count'), 'REVIEW=', (d.get('confidence_counts') or {}).get('review'))
...
print('V11 STAGE 1 EXACT-READY QA PASSED')
PY

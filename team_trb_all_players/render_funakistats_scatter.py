...
and label placement are deterministic and separately auditable.
"""
...
def _annotation(ax,row,x,y,player,dx,dy):
    ...
    return ax.annotate(str(row[player]),xy=(float(row[x]),float(row[y])),xytext=(dx,dy),textcoords='offset points',ha=ha,va=va,fontsize=9.2,fontweight='semibold',annotation_clip=True,arrowprops=dict(arrowstyle='-',linewidth=.72,shrinkA=3,shrinkB=5,connectionstyle='arc3,rad=0'),zorder=7)
...
def render(df,x,y,player,minutes,out,title,subtitle,xlabel,ylabel,threshold=10000,label_count=20,editorial_players=()):
...
def main():
    ...
    print(json.dumps(render(d,a.x,a.y,a.player,a.minutes,a.output_prefix,a.title,a.subtitle,a.x_label or a.x,a.y_label or a.y,a.minutes_threshold,a.label_count,a.editorial_player),indent=2))

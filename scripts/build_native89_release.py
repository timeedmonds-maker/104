...
on=pick(['on','on_value','ON','value_on'])
off=pick(['off_corrected','off','off_value','OFF','value_off'])
swing=pick(['on_minus_off_corrected','swing','swing_value','SWING','value_swing'])
if not all([season,pid,tid,metric,on,off,swing]):
...
print('RESOLVED',season,pid,tid,metric,on,off,swing)
def is_treb(x):
...
print('METRICS',len(metrics)); print('KEYS',len(keys)); print('NATIVE_ROWS',len(native)); print('UNIQUE_PAIRS',len(pairs))
...
outrows=[]; bad=0
for r in native:
    a=r.get(on); b=r.get(off); s=r.get(swing)
    if a is None or b is None or s is None: raise SystemExit('missing ON/OFF/SWING value')
    if isinstance(a,(int,float)) and isinstance(b,(int,float)) and isinstance(s,(int,float)):
        if abs(s-(a-b))>1e-9: bad+=1
